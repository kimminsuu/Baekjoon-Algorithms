...
def rec(temp, i, j, d, deq,fillin) :
    if temp > 0 and (i,j,temp) in deq:
        ni = i + di[d]
        nj = j + dj[d]
            ...
            if fillin:
                if 0 <= ni1 < R and 0 <= nj1 < C:
                    if (i, j, 0) not in byuk:
                        rec(temp,ni1,nj1,d,deq,False)
                if 0 <= ni2 < R and 0 <= nj2 < C:
                    if (ni2, nj2, 0) not in byuk:
                        rec(temp, ni2, nj2, d, deq,False)
        else :
            ni1 = i + di[0]
            nj1 = j + dj[0]
            ni2 = i + di[1]
